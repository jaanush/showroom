...
(function($, o){
	...
	$.extend(o, {
		...
		constants: {
			scrollDuration: 800,
			captureThreshold: 7,//4,
			wheelDeltaMod: -18,
			scrollDeltaMod: 10,//5.7,
			thumbThickness: 8,
			thumbOpacity: 0.7,
			boundingBox: 1000000
		},
		...
		wheel: function(event, delta) {
			...
			if(event.data.thumbs) event.data.thumbs.vertical.stop(true, true).fadeTo(0, o.constants.thumbOpacity);
			event.data.target.scrollTop(event.data.target.scrollTop() - (delta * o.constants.wheelDeltaMod));
			if(event.data.thumbs) event.data.thumbs.vertical.stop(true, true).fadeTo("fast", 0);

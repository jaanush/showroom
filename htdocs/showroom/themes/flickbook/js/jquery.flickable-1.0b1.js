...
(function($, window, Math, undefined) {
	...
	var flickable, 
		...
		touchEvent = eventSupported('touchstart');
	...
	flickable.prototype = {
		...
		create: function(options, elem) {
			...
			if (elem == window || elem == document || /^(html|body)$/i.test(elem.nodeName)) {
				// we don't support flick scrolling the entire page on touch screen
		...
		init: function() {
			...
			if (!this.container) {
				...
				this.container = element.children().bind((touchEvent ? 'touchstart' : 'mousedown') + '.flickable', function(event) {
					return self.dragStart(event);
				});
				this.wrapper = this.container.children();
				this.content = this.wrapper.children();

				this.elementWrapper.bind((touchEvent ? 'touchstart' : 'mousedown') + '.flickable keydown.flickable', function() {
					self.deactivate();
				}).bind((touchEvent ? 'touchend' : 'mouseup') + '.flickable mouseleave.flickable keyup.flickable', function() {
					self.activate();
				});
		...
		setOption: function(key, value) {
			...
			switch (key) {
				case 'cancel':
					...
					this.cancel.addClass('ui-flickable-canceled').bind((touchEvent ? 'touchstart' : 'mouseenter') + '.flickable', function() {
						self.canceled = true;
					}).bind((touchEvent ? 'touchend' : 'mouseleave') + '.flickable', function() {
						self.canceled = false;
					});
					break;
				case 'disabled':
		...
		dragStart: function(event) {
			var self = this,
				e = touchEvent ? event.originalEvent.touches[0] : event;
			...
			if (!this.canceled && !this.options.disabled) {
				...
				this.container.unbind('.flickable').bind((touchEvent ? 'touchmove' : 'mousemove') + '.flickable', function(event) {
					return self.drag(event);
				}).bind((touchEvent ? 'touchend' : 'mouseup') + '.flickable mouseleave.flickable', function(event) {
					return self.dragStop(event);
				});

				if (!/^(a|input|textarea|select|option|button|embed|object)$/.test(e.target.nodeName.toLowerCase())) {
		...
		drag: function(event) {
			...
			if (!/^(input|textarea|select|option|button|embed|object)$/.test(e.target.nodeName.toLowerCase())) {
				event.preventDefault();
			}
		},
		...
		dragStop: function(event) {
			...
			this.container.unbind('.flickable')
				.bind((touchEvent ? 'touchstart' : 'mousedown') + '.flickable', function(event) {
					return self.dragStart(event);
				});
		...
		flick: function() {
			...
			if (this.sections) {
				...
				if (inertialVelocity.x || inertialVelocity.y) {
			...
			this.inertia = setInterval(function() {
				if (options.disabled || (!inertialVelocity.y && !inertialVelocity.x) ||
					self.trigger('flick') === false) {
		...
		scrollBack: function() {
			...
			this.back = setInterval(function() {
				...
				if (self.sections) {
					...
					if (!extension.x && !extension.y && !inertialVelocity.x && !inertialVelocity.y) {
		...
		select: function(index) {
			...
			this.inertia = setInterval(function() {
				...
				if (self.options.disabled || (!inertialVelocity.y && !inertialVelocity.x)) {
					clearInterval(self.inertia);
					self.scrollBack();
					return;
				}
